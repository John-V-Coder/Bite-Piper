...
# -----------------------------
# Convert JSON / CSV to MeTTa facts
# -----------------------------
def json_to_facts(data, parent_key=""):
    ...
    if isinstance(data, dict):
        ...
        key_clean = full_key.replace(" ", "_").replace("-", "_")
...
# Load company data
# -----------------------------
company_data_path = "socioeconomic_supply_chain_data.json"  # Replace with your file

if company_data_path.endswith(".json"):
    with open(company_data_path, "r") as f:
        company_data = json.load(f)
elif company_data_path.endswith((".xls", ".xlsx", ".csv")):
...
response_stream = agent(f'(user "{query}")',
                        additional_info=[
